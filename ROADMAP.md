...
|---|---|---|
| A | Metric definitions doc (M1–M11 formulas, edge cases); annotation spec (room polygons, adjacency, door positions, per-room areas); collect first 10 listings internally; **start partner outreach week 1, day 1** — now targeted per [`docs/DATA-SOURCES.md`](docs/DATA-SOURCES.md): approach agents *by which CRM they use* (Street.co.uk / Reapit in the UK, Apimo / Hektor in France), since those expose OAuth APIs with photos and floor plans. Also week 1: **email the Rent3D authors** for licence terms (215 London flats with photo↔plan alignment) | Spec reviewed; 10 listings ingested; ≥3 agent conversations booked; Rent3D enquiry sent |
