...
- [ ] Data partner: signed LOI **or** two agencies in active negotiation with plausible close < 4 weeks. **Kill criterion (from the report, kept):** no partner and <30 GT listings ⇒ stop; without a data partner this project is much harder than it needs to be — pivot or shelve.
...
| Stream | Work |
|---|---|
| C | Plan vectorisation v1: preprocessing (deskew, binarise) + wall/room extraction. Model plan: start from RoomFormer-class architecture (code MIT) but **train on partner plans + our annotations + a synthetic plan generator** — CubiCasa5K/Structured3D are NC (ledger) and must not taint the shipped model. Annotation tooling for plans lands this sprint (A helps). Target for now: ≥80% room F1 on our plan validation split. |
